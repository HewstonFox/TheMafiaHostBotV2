...
class MessageController:
    ...
    async def send_registration_start(cls, chat_id: ChatId, t: Localization):
        ...
        return await bot.send_message(chat_id, t.group.registration.start, reply_markup=inline_keyboard)
    ...
    async def send_registration_reminder(cls, chat_id: ChatId, t: Localization, time: int, reply_id: ChatId):
        inline_keyboard = arr2keyword_markup([[{'text': 'Connect', 'callback_data': CallbackQueryActions.add_player}]])
        return await bot.send_message(chat_id, t.group.registration.reminder.format(time), reply_markup=inline_keyboard,
                                      reply_to_message_id=reply_id)

    @classmethod
    async def send_registration_force_stopped(cls, chat_id: ChatId, t: Localization):
        return await bot.send_message(chat_id, t.group.registration.force_stopped)

    @classmethod
    async def send_registration_skipped(cls, chat_id: ChatId, t: Localization):
        return await bot.send_message(chat_id, t.group.registration.skipped)

    @classmethod
    async def send_nothing_to_stop(cls, chat_id: ChatId, t: Localization):
        return await bot.send_message(chat_id, t.group.nothing_to_stop)

    @classmethod
    async def send_user_connected_to_game(cls, chat_id: ChatId, session: Session):
        return await bot.send_message(chat_id, session.t.private.user_connected.format(session.name))

    @classmethod
    async def send_user_left_game(cls, chat_id: ChatId, session: Session):
        return await bot.send_message(chat_id, session.t.private.user_left.format(session.name))
